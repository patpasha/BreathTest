...
const RespirationDiaphragmatiqueScreen = ({ route, navigation }: BreathingScreenProps) => {
  ...
  const steps = [
    { name: 'Inspiration', duration: 4000, instruction: 'Inspirez lentement par le nez en gonflant le ventre' },
    { name: 'Pause', duration: 1000, instruction: 'Pause brève' },
    { name: 'Expiration', duration: 6000, instruction: 'Expirez lentement par la bouche en rentrant le ventre' },
  ...
  const handleStop = async () => {
    ...
    if (sessionStartTime) {
      ...
      console.log('Session Respiration Diaphragmatique interrompue après', sessionDuration, 'secondes'); // Debug
      
      // N'enregistrer que si la session a duré au moins 10 secondes
      if (sessionDuration >= 10) {
        try {
          await addSession({
            techniqueId: 'respiration-diaphragmatique',
            techniqueName: 'Respiration Diaphragmatique',
            duration: sessionDuration,
            date: new Date().toISOString(),
            completed: false
          });
          console.log('Session Respiration Diaphragmatique interrompue enregistrée avec succès'); // Debug
        } catch (error) {
  ...
  const handleComplete = () => {
    ...
    Alert.alert(
      "Session terminée",
      `Vous avez complété ${currentCycle} cycles de respiration diaphragmatique.`,
      [{ text: "OK" }]
    );
  };
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
        ...
        >
          ...
          <View style={styles.instructionContainer}>
            <Text style={[styles.instructionTitle, { color: theme.textPrimary }]}>Respiration Diaphragmatique</Text>
            <Text style={[styles.instructionText, { color: theme.textSecondary }]}>
              La respiration diaphragmatique, aussi appelée respiration abdominale, est une technique fondamentale qui utilise le diaphragme pour une respiration profonde et efficace.
            </Text>
            <Text style={[styles.instructionText, { color: theme.textSecondary }]}>
              1. Inspirez lentement par le nez en gonflant le ventre (pas la poitrine).
            </Text>
            <Text style={[styles.instructionText, { color: theme.textSecondary }]}>
            ...
            <Text style={[styles.instructionText, { color: theme.textSecondary }]}>
              4. Répétez ce cycle pour réduire le stress, améliorer l'oxygénation et activer le système nerveux parasympathique.
            </Text>
          </View>
...
export default RespirationDiaphragmatiqueScreen;
