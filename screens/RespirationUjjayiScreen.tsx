...
const RespirationUjjayiScreen = ({ route, navigation }: BreathingScreenProps) => {
  ...
  const handleStop = async () => {
    ...
    if (sessionStartTime) {
      ...
      console.log('Session Respiration Ujjayi interrompue après', sessionDuration, 'secondes'); // Debug
      
      // N'enregistrer que si la session a duré au moins 10 secondes
      if (sessionDuration >= 10) {
        try {
          await addSession({
            techniqueId: 'respiration-ujjayi',
            techniqueName: 'Respiration Ujjayi',
            duration: sessionDuration,
            date: new Date().toISOString(),
            completed: false
          });
          console.log('Session Respiration Ujjayi interrompue enregistrée avec succès'); // Debug
        } catch (error) {
